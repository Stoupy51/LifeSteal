...
#
# @within	advancement life_steal:consume_crafted_heart
#

# Revoke the advancement
advancement revoke @s only life_steal:consume_crafted_heart
...
execute if score @s life_steal.hearts >= #temp life_steal.data unless score MAX_HEARTS_BY_CONSUMING life_steal.data = MAX_HEARTS life_steal.data run tellraw @s {"text":"You are already at max health from consuming crafted hearts! You can only gain more hearts by killing players.","color":"red"}
execute if score @s life_steal.hearts >= #temp life_steal.data at @s run playsound entity.villager.no ambient @s
execute if score @s life_steal.hearts >= #temp life_steal.data at @s run loot spawn ~ ~ ~ loot life_steal:i/crafted_heart
